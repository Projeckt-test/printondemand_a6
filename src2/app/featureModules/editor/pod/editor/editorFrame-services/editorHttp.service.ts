...
export class editorHttpService {
    ...
    constructor(public http: HttpClient, private appConfig: AppConfig) {
        this.standard = localStorage.getItem('projectstandard');
    }
    ...
    createHtml = async function (obj, filename) {
        try {
            ...
            await this.http.post(this.appConfig.config.apiURL + "/createNewhtml", { head: obj.head, body: obj.body, url: obj.url, pagewidth: viewerwidth, pageheight: viewerheight, standard: this.standard }).toPromise();
