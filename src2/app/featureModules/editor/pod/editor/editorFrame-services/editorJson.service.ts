...
export class editorJsonService {
  ...
  setCurrentElementAttr = async function (json, element) {
    ...
    // Add custom height and width for image 
    if(element && element.nodeName.toLowerCase() == "img" && !(element.getAttribute('custom_width') && element.getAttribute('custom_height'))){
      json += ',"custom_width":"' + (element.getAttribute('custom_width') ? element.getAttribute('custom_width') : (element.getAttribute('width') ? element.getAttribute('width') : "")) + '"';
      json += ',"custom_height":"' + (element.getAttribute('custom_height') ? element.getAttribute('custom_height') : (element.getAttribute('height') ? element.getAttribute('height') : "")) + '"';
    }
