...
import { StyleDropdownDialogComponent } from './style-dropdown-dialog/style-dropdown-dialog.component';
...
export class EditorComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit() {
    ...
    let id = this.editorDomService.sendId
      .subscribe(item => this.findHtmlElement(item.id, item.event, item.tagName));
  ...
  /* If User have an edit access to that project then shows edit button Otherwise hide the edit button in the viewer screen */
  ...
  styleDropDownDialog(){
    const dialogRef = this.dialog.open(StyleDropdownDialogComponent, {
      width: '45%',
      // data: { "element": elem}
    });
